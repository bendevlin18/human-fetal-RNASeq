...
server <- function (input, output, session) {
  ...
  output$landing_text <- renderUI ({
    ...
    x <- list(tags$h2("Welcome to Human Fetal RNA Seq!", style = 'text-align: center'),
              ...
              img(src="microglia.gif", height = '200px', width='252px', style = "display: block; margin-left: auto; margin-right: auto;"),
              tags$h5(""),
  ...
  output$background_text <- renderUI({
    aa <- list(tags$h1("The Backstory", style='text-align: center'),
      ...
      HTML(paste("Recently, we uncovered that maternal high fat diet leads to endotoxin accumulation
          ...
          img(src="humanVSmouse.png", height = '50px', width='233px', style = "display: block; margin-left: auto; margin-right: auto;"),
      ...
      img(src="summary.png", height="145px", width="310px",style = "display: block; margin-left: auto; margin-right: auto;"))
    tagList(aa)
  })
  ...
  output$plot1 <- renderPlotly({
    ...
    if (dim(goi_df_placenta)[1]==0 && dim(goi_df_brain)[1]==0) {
      ...
      malebrainpval <- round(malebrainStat$p.value,2)
      ...
      femalebrainpval <- round(femalebrainStat$p.value,2)
      ...
      Female_Brain_Corr <- ggplotly(ggplot(female_brain_data, aes(x = Maternal_Triglycerides, y = Brain))+
                                      ...
                                      corr_plot_settings1)
      brplot3 <- ggplotly(ggplot())
      
      splitCorrLayout <- subplot(Female_Brain_Corr, brplot3, Male_Brain_Corr, nrows=3, titleY=TRUE, titleX = TRUE,
      ...
      ggplotly(splitCorrLayout) %>% layout(annotations = annotations, autosize = F, width = 300, height = 563)
      ...
      malePlacentapval <- round(malePlacentaStat$p.value,2)
      ...
      femalePlacentapval <- round(femalePlacentaStat$p.value,2)
      ...
      brplot4 <- ggplotly(ggplot())
      
      splitCorrLayout <- subplot(Female_Plac_Corr, brplot4, Male_Plac_Corr, nrows=3, titleY=TRUE, titleX = TRUE,
                                 heights=c(0.45, 0.1, 0.45)) 
      ...
      Male_Brain_Corr <- ggplotly(ggplot(male_brain_data, aes(x = Maternal_Triglycerides, y = Brain))+
                                    ...
                                    geom_line(stat="smooth", method="lm",color = "lightcyan4",
                                              size=.5)+
                                    ggtitle(label = gene_of_interest)+
                                    geom_text(x=.95, y=malebrain_TPM_max, aes(label=paste(malebrainR,",",malebrainpval)), size=2.2)+
                                    corr_plot_settings4)
      ...
      Female_Brain_Corr <- ggplotly(ggplot(female_brain_data, aes(x = Maternal_Triglycerides, y = Brain))+
                                      ...
                                      geom_line(stat="smooth", method="lm",color = "rosybrown",
                                                size=.5)+
                                      geom_text(x=1.15, y=femalebrain_TPM_max, aes(label=paste(femalebrainR,",",femalebrainpval)), size=2.2)+
                                      corr_plot_settings3)
      ...
      Male_Plac_Corr <- ggplotly(ggplot(male_plac_data, aes(x = Maternal_Triglycerides, y = Placenta))+
                                   ...
                                   geom_line(stat="smooth", method="lm",color = "lightcyan4",
                                             size=.5)+
                                   geom_text(x=.95, y=malePlacenta_TPM_max, aes(label=paste(malePlacentaR,",",malePlacentapval)), size=2.2)+
                                   corr_plot_settings2)
      ...
      Female_Plac_Corr <- ggplotly(ggplot(female_plac_data, aes(x = Maternal_Triglycerides, y = Placenta))+
                                     ...
                                     geom_line(stat="smooth", method="lm",color = "rosybrown",
                                               size=.5)+
                                     geom_text(x=1.15, y=femalePlacenta_TPM_max, aes(label=paste(femalePlacentaR,",",femalePlacentapval)), size=2.2)+
                                     corr_plot_settings1)
      ...
      splitCorrLayout <- subplot(Female_Plac_Corr, brplot, Male_Plac_Corr, brplot, brplot, brplot, Female_Brain_Corr, brplot, Male_Brain_Corr, nrows=3, 
                                 titleY=TRUE, titleX = TRUE, margin=0.04, heights = c(0.45, 0.1, 0.45), widths = c(0.475, 0.05, 0.475))
      ...
      ggplotly(splitCorrLayout) %>% layout(annotations = annotations, autosize = F, width = 400, height = 375)
    }
  })
